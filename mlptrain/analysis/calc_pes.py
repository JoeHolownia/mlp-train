...
def calculate_pes(
    ...
    # traj_product_optimised.save_xyz(product_fname, predicted=True)
    ...
    if os.path.exists(npz_fname):
        logger.info(f'Loading existing PES file: {npz_fname}. Delete or move this file if you wish to run this scan again...')
        pes.load(npz_fname)
    else:
        # calculate the pes
        pes.calculate(

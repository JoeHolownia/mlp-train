...
def mlpmd_fix_solute(
    ...
    logger.info(
        'Run MLP MD with fixed solute (solute coords should at the first in configuration coords) by MLP'
    )
    ...
    dyn = Langevin(
        ase_atoms, dt * ase_units.fs, temperature_K=temp, friction=0.02
    )
...
def optimize_sys(configuration, mlp, **kwargs):
    ...
    logger.info(
        'Optimise the configuratoin with fixed solute (solute coords should at the first in configuration coords) by MLP'
    )
...
def get_reactant_states(TS, solution, mlp):
    ...
    solved_in_solution = solvation(
        solute_config=TS, solvent_config=solution, apm=3, radius=1.7
    )

    after_fixed_md = mlpmd_fix_solute(
        solute=TS,
    ...
    trajectory_reactant = mlt.md.run_mlp_md(
        configuration=after_fixed_md[-1],
        mlp=mlp,
        fs=200,
        temp=300,
        dt=0.5,
    ...
    rt1 = np.linalg.norm(
        opt_reactant.atoms[1].coord - opt_reactant.atoms[12].coord
    )
    rt2 = np.linalg.norm(
        opt_reactant.atoms[6].coord - opt_reactant.atoms[11].coord
    )
    logger.info(
        f'the forming carbon bonds length in reactant are {rt1}, {rt2}'
    )
    return opt_reactant
...
def generate_rs(TS, solution, mlp, box_size):
    ...
    while len(reactants) < 10:
        reactant = get_reactant_states(TS=TS, solution=solution, mlp=mlp)
        rt1 = np.linalg.norm(
            reactant.atoms[1].coord - reactant.atoms[12].coord
        )
        rt2 = np.linalg.norm(
            reactant.atoms[6].coord - reactant.atoms[11].coord
        )
    ...
    for i, species in enumerate(reactants):
        bias = mlt.Bias(
            zeta_func=mlt.AverageDistance((1, 12), (6, 11)),
            kappa=0.5,
            reference=ref[i],
        )
        traj = baised_md(

...
def grid_box(positions, size, grid_space):
    ...
    grid_points = (
        np.vstack(np.meshgrid(x_space, y_space, z_space)).reshape(3, -1).T
    )
    return grid_points
...
def cavity_volume(ase_system, solute_idx=22, grid_side_length=0.2, radius=1.5):
    ...
    grid_pos = grid_box(
        positions=solute_sys.get_positions(),
        size=2.0,
        grid_space=grid_side_length,
    )
...
def traj_study(configs, ml_potential, init_md_time_fs=500, max_time_fs=3000):
    ...
    for k in range(500):
        ...
        while tol_md_time_f <= max_time_fs:
            ...
            traj, reaction_coords, cavity_volume = md_with_file(
                config,
                mlp=ml_potential,
                temp=300,
                dt=0.5,
